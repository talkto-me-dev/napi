...
const github_repo = execSync("git remote get-url origin").toString().trim().split(':').pop().replace(/\.git$/, '').replace('443/','');
...
const snake = project.replaceAll('-', '_'),
  ...
  MAP = {
    ...
    'lib.rs': subS,
    'index.test.js': subS,
    'index.js': (c) => subS(c).replaceAll(snake + '.node', project + '.node')
  },
  proc = (path) => {
